      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-16">
          {securityFeatures.map((section) => (
            <div key={section.title} className="p-6 rounded-xl border border-[#1A2D44] bg-[#0A1628]/50">
              <section.icon className="h-8 w-8 text-[#00E3FD] mb-4" />
              <h3 className="text-lg font-semibold mb-4">{section.title}</h3>
              <ul className="space-y-2.5">
                {section.items.map((item) => (
                  <li key={item} className="flex items-start gap-2 text-sm text-[#6B7D95]">
        ...
        {/* Certifications */}
        <div className="p-8 rounded-xl border border-[#1A2D44] bg-[#0A1628]/50 mb-16">
          <h2 className="text-2xl font-bold mb-6">Certifications & Compliance</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {certifications.map((cert) => (
              <div key={cert.name} className="p-4 rounded-lg border border-[#1A2D44] bg-[#060B14]/50 text-center">
                <div className="text-2xl mb-2">{cert.icon}</div>
                <div className="font-semibold text-sm mb-1">{cert.name}</div>
                <div className="text-xs text-[#6B7D95]">{cert.status}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Uptime & Monitoring */}
        <div className="grid md:grid-cols-2 gap-6 mb-16">
          <div className="p-6 rounded-xl border border-[#1A2D44] bg-[#0A1628]/50">
            <Clock className="h-8 w-8 text-[#00E3FD] mb-4" />
            <h3 className="text-lg font-semibold mb-2">Uptime SLA</h3>
            <div className="text-4xl font-bold text-[#00E3FD] mb-2">99.9%</div>
            <p className="text-sm text-[#6B7D95]">Enterprise plan includes 99.9% uptime SLA with financial penalties.</p>
          </div>
          <div className="p-6 rounded-xl border border-[#1A2D44] bg-[#0A1628]/50">
            <AlertTriangle className="h-8 w-8 text-[#00E3FD] mb-4" />
            ...
            <Link href="/trust/disclosure" className="text-sm text-[#00E3FD] hover:underline inline-flex items-center gap-1">
        ...
        {/* Contact */}
        <div className="text-center p-8 rounded-xl border border-[#1A2D44] bg-[#0A1628]/50">
          <Shield className="h-10 w-10 text-[#00E3FD] mx-auto mb-4" />
